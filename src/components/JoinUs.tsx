...
export default function JoinUs({ blockData, id }: { blockData: any, id: any }) {
    ...
    <div className='max-w-7xl mx-auto xs:px-10 xxs:px-0 sm:mb-[200px] xxs:mb-[100px] mb-[50px]' key={id}>
        <div className='bg-navy py-[80px] md:px-[315px] sm:px-[215px] xs:px-[100px] xxs:px-0  xxs:rounded-none xs:rounded-[70px]'>
            <div className='flex flex-col justify-center items-center text-center'>
                <h1 className='text-white mb-3 tracking-normal w-full'>{blockData.title}</h1>
                <p className='text-white'>{blockData.body}</p>
                <div className='mt-[32px]'>
                    <Link href={`${blockData.button.href}`}>
                        <button className={`font-Jost flex justify-center items-center rounded-[18px] bg-brown hover:opacity-60 transition duration-300 
