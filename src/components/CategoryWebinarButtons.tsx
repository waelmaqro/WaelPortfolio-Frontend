...
export default function CategoryWebinarButtons({ blockData }: { blockData: any }) {
    ...
    <div className='flex flex-col justify-center items-center'>
      <div className='max-w-7xl'>
        {categoriesToShow.map((item: any, index: number) => (
          ...
          >
            <Link href={`/webinars/${item.attributes.slug}`}>{item.attributes.Category}</Link>
          </button>
        ))}
      </div>
